% Function: recover the translation
% Input: A, B: string of the image path
% Output: string of the output image path
function img_recover_trans = trans_recover(A, B)
% Read image
temp1 = imread(A);
temp2 = imread(B);
...
%% 当B很大时选择裁剪B
% This is executed if A and B have similar size  A和B形状相似时执行
...
if xoffset > 0 && yoffset > 0
    ...
    image_B_recover(ybegin:yend,xbegin:xend,:) = image_B;
    image_A_recover = image_A;
elseif xoffset < 0 && yoffset > 0
    xoffset = -xoffset;
    xbegin = round(xoffset + 1);
    xend   = round(xoffset + size(image_A,2));
    ybegin = 1;
    yend   = round(size(image_A,1));
    image_A_recover(ybegin:yend,xbegin:xend,:) = image_A;
    xbegin = 1;
    xend   = round(size(image_B,2));
    ybegin = round(yoffset + 1);
    yend   = round(yoffset + size(image_B,1));
    image_B_recover(ybegin:yend,xbegin:xend,:) = image_B;
end
...
% 
imshowpair(image_A_recover,image_B_recover,"blend")
% img_recover_trans = "./img_src/img_recover_trans.png";
% imwrite(recovered_image, img_recover_trans);
